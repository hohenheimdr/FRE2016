...
ofstream myfile;
	...
	PathWriter(double frequency,std::string frame_id,bool record_file)
	{
		...
		ROS_INFO("path writer was published");
...
int main(int argc, char **argv)
{
	...
	PathWriter path(f,frame,record_file);
	
	n.param<double>("odometry_error",path.odometry_error,0.1);
	path.file=file;
	ros::Subscriber odometry_sub = n.subscribe(odom_sub_topic,15,&PathWriter::check_Position,&path);
		
	path.path_publisher=n.advertise<nav_msgs::Path>(path_pub_topic.c_str(), 100);
