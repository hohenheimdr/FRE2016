 ...
 * changed the code for row filtering of a normal laser scanner..
 * some aditional tools had been aplied at 1.6.16
 * 
 * 
 *  * */
...
// public variables
	...
	void readFrontCloud(const sensor_msgs::PointCloud2::ConstPtr& msg)
	{
		...
		//apply push to plane
		input=push_to_plane(input,conf.z_value);
	...
	pcl::PointCloud<pcl::PointXYZI>::Ptr box_filter(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud)
	{
		...
		if(conf.use_box_filter)
		{
			...
			{
				if(cloud->points[i].x>conf.x_min && cloud->points[i].x<conf.x_max)
					if(cloud->points[i].y>conf.y_min && cloud->points[i].y<conf.y_max)
						if(cloud->points[i].z>conf.z_min && cloud->points[i].z<conf.z_max)
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr min_cluster_clouds(pcl::PointCloud<pcl::PointXYZ>::Ptr input)
	{
		if(conf.use_mincluster)
		{
		  pcl::PointCloud<pcl::PointXYZ>::Ptr center_points (new pcl::PointCloud<pcl::PointXYZ>);
		  // Creating the KdTree object for the search method of the extraction
		  pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ>);
		  ...
		  std::vector<pcl::PointIndices> cluster_indices;
		  pcl::EuclideanClusterExtraction<pcl::PointXYZ> ec;
		  ec.setClusterTolerance (0.1); // in cm
